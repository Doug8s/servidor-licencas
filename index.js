...
app.post("/api/activate", async (req, res) => {
  ...
  if (!key || !machineId) {
    return res
      ...
      .json({
        success: false,
        message: "Chave e ID da máquina são obrigatórios.",
      });
  }
  ...
  try {
    ...
    if (!license) {
      return res
        .status(404)
        .json({ success: false, message: "Chave de licença não encontrada." });
    }
    ...
    if (license.isActivated && license.machineId === machineId) {
      return res
        .status(200)
        .json({ success: true, message: "Licença revalidada com sucesso." });
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Aplicativo ativado com sucesso!" });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
